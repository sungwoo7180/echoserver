...
public class NioEchoClient2 {
    ...
    private static final int MODE = 1; // 1 = 거절 모드, 2 = 쪼개기 모드
            ...
            while (true) {
                ...
                if (!waitingForResponse) {
                    ...
                    if (userInput.isEmpty()) {
                        ...
                        continue; // 입력이 비었으면 다시 입력받도록
                    }

                    if (MODE == 1) {
                        if (userInput.length() > MAX_MESSAGE_SIZE) {
                            System.out.println("!!메시지가 너무 큽니다! 최대 " + MAX_MESSAGE_SIZE + "자까지 가능합니다.");
                            continue;
                        }
                        waitingForResponse = true;
                        handleWrite(socketChannel, buffer, userInput);
                    } else if (MODE == 2) {
                        waitingForResponse = true;
                        sendChunkedMessage(socketChannel, buffer, userInput);
                    }
                }
            }
        } catch (IOException e) {
    ...
    private static boolean handleRead(SelectionKey key, ByteBuffer buffer) throws IOException {
        ...
        buffer.clear(); // 버퍼 초기화
        return false; // 서버 응답을 받았으므로 대기 상태 해제
    }
